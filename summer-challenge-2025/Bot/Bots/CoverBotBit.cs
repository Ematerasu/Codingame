...
public class CoverBotBit : AI
{
    ...
    public override TurnCommand GetMove(GameState state)
    {
        ...
        for (int id = 0; id < GameState.MaxAgents; ++id)
        {
            ...
            var cls = _myClass[id];
            var myStats = AgentUtils.Stats[GameState.AgentClasses[id]];
            ...
            for (int i = 0; i < legal; i++)
            {
                ...
                int advance = PlayerId == 0 ? tx : (state.W - 1 - tx);
                ...
                for (int j = 0; j < GameState.MaxAgents; ++j)
                {
                    ...
                    int d = GameState.Mdist(tx, ty, ally.X, ally.Y);
                    ...
                    case CombatType.Shoot:
                        {
                            ...
                            if (!en.Alive) break;
                            seesEnemy = true;
                            int dist = GameState.Mdist(tx, ty, en.X, en.Y);
    ...
    private (int dist, double bonus) EvaluateSeekTarget(GameState st, int x, int y, AgentStats myStat, AgentClass cls)
    {
        ...
        for (int i = 0; i < GameState.MaxAgents; ++i)
        {
            ...
            int dist = GameState.Mdist(x, y, en.X, en.Y);
    ...
    private void RecogniseClasses(GameState st)
    {
        for (int id = 0; id < GameState.MaxAgents; ++id)
        {
            ...
            var s = AgentUtils.Stats[GameState.AgentClasses[id]];
            _myClass[id] = AgentUtils.GuessClass(s.ShootCooldown, s.OptimalRange, s.SoakingPower, ag.SplashBombs);
        }
    }
    ...
    private void PrecomputeCover(GameState st)
    {
        ...
        for (int y = 0; y < _h; y++)
            for (int x = 0; x < _w; x++)
            {
                if (GameState.Tiles[GameState.ToIndex(x, y)] != TileType.Empty) { _coverScore[x, y] = -9999; continue; }
                double s = 0;
                foreach (var (dx, dy) in dirs4)
                {
                    int nx = x + dx, ny = y + dy;
                    if (!st.InBounds(nx, ny)) continue;
                    var t = GameState.Tiles[GameState.ToIndex(nx, ny)];
                    if (t == TileType.HighCover) s += 4;
                    else if (t == TileType.LowCover) s += 2;
                }
                _coverScore[x, y] = s;
            }
    }
    ...
    private void PrecomputeSniperTargets(GameState st)
    {
        for (int id = 0; id < GameState.MaxAgents; ++id)
        {
            ...
            for (int x = 0; x < _w; x++)
            {
                double val = _coverScore[x, st.Agents[id].Y];
                if (val < 2) continue;
                int cnt = 0;
                for (int cy = 0; cy < _h; cy++)
                    if (GameState.Mdist(x, cy, x, st.Agents[id].Y) <= AgentUtils.Stats[AgentClass.Sniper].OptimalRange) cnt++;
    ...
    private bool AnyEnemyVisible(GameState st)
    {
        for (int i = 0; i < GameState.MaxAgents; ++i)
        {
            ...
            for (int j = 0; j < GameState.MaxAgents; ++j)
            {
                ...
                if (GameState.Mdist(ag.X, ag.Y, en.X, en.Y) <= stt.OptimalRange * 2) return true;
            }
        }
        return false;
    }
    ...
    private bool EnemyCanShoot(GameState st, int x, int y)
    {
        for (int i = 0; i < GameState.MaxAgents; ++i)
        {
            ref readonly var en = ref st.Agents[i];
            if (!en.Alive || en.playerId == PlayerId || en.Cooldown > 0) continue;
            if (GameState.Mdist(x, y, en.X, en.Y) <= AgentUtils.Stats[GameState.AgentClasses[i]].OptimalRange * 2) return true;
        }
        return false;
    }
    ...
    private bool IsBehindCover(GameState st, int sx, int sy, in AgentState t)
    {
        ...
        if (Math.Abs(dx) > 1)
        {
            int cx = t.X - Math.Sign(dx), cy = t.Y;
            if (st.InBounds(cx, cy) && GameState.Tiles[GameState.ToIndex(cx, cy)] != TileType.Empty) return true;
        }
        if (Math.Abs(dy) > 1)
        {
            int cx = t.X, cy = t.Y - Math.Sign(dy);
            if (st.InBounds(cx, cy) && GameState.Tiles[GameState.ToIndex(cx, cy)] != TileType.Empty) return true;
        }
        return false;
    }
